...
func RecordUsage(account *billing.AccountInfo, usageRecords []billing.UsageRecord) error {
	for _, record := range usageRecords {
		...
		if record.AppInst == nil && record.ClusterInst == nil {
			...
			memo = fmt.Sprintf("{%s}, Flavor: %s, start: %s, end %s", record.ClusterInst.String(), record.FlavorName, record.StartTime.Format(time.RFC3339), record.EndTime.Format(time.RFC3339))
		} else { //record.ClusterInst == nil
			cloudlet = &record.AppInst.ClusterInstKey.CloudletKey
			memo = fmt.Sprintf("{%s}, Flavor: %s, NumNodes %d, start: %s, end %s", record.AppInst.String(), record.FlavorName, record.NodeCount, record.StartTime.Format(time.RFC3339), record.EndTime.Format(time.RFC3339))
		}
