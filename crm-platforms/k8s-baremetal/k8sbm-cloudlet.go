...
func (k *K8sBareMetalPlatform) CreateCloudlet(ctx context.Context, cloudlet *edgeproto.Cloudlet, pfConfig *edgeproto.PlatformConfig, flavor *edgeproto.Flavor, caches *platform.Caches, accessApi platform.AccessApi, updateCallback edgeproto.CacheUpdateCallback) (bool, error) {
	...
	if k.commonPf.ChefClient == nil {
		return cleanupOnError, fmt.Errorf("Chef client is not initialized")
	}
	...
	if cloudlet.InfraApiAccess == edgeproto.InfraApiAccess_RESTRICTED_ACCESS {
		return cleanupOnError, fmt.Errorf("Restricted access not yet supported on BareMetal")
	}
	...
	if err != nil {
		return cleanupOnError, fmt.Errorf("Failed to get ssh client to control host: %v", err)
	}
	if pfConfig.CrmAccessPrivateKey != "" {
		...
		if err != nil {
			return cleanupOnError, fmt.Errorf("Write access key fail: %v", err)
		}
	}
	...
	return cleanupOnError, chefmgmt.GetChefRunStatus(ctx, k.commonPf.ChefClient, clientName, cloudlet, pfConfig, accessApi, updateCallback)
}
