...
func (s *Platform) Init(ctx context.Context, platformConfig *platform.PlatformConfig, updateCallback edgeproto.CacheUpdateCallback) error {
	...
	err = mexos.SetupRootLB(ctx, rootLBName, vmspec, platformConfig.CloudletKey, mexos.RootLBVMDeployment, platformConfig.ImagePath, platformConfig.ImageVersion, edgeproto.DummyUpdateCallback)
	if err != nil {
		return err
	}
