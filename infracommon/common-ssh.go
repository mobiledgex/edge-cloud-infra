...
func (c *CommonPlatform) GetSSHClient(ctx context.Context, serverName, networkName, userName string, ops ...SSHClientOp) (ssh.Client, error) {
	log.SpanLog(ctx, log.DebugLevelMexos, "GetSSHClient", "serverName", serverName)
	opts := SSHOptions{Timeout: DefaultConnectTimeout}
	opts.Apply(ops)

	addr, err := c.infraProvider.GetIPFromServerName(ctx, networkName, serverName)
	if err != nil {
		return nil, err
	}
...
func (c *CommonPlatform) SetupSSHUser(ctx context.Context, rootLB *MEXRootLB, user string) (ssh.Client, error) {
	...
	client, err := c.GetSSHClient(ctx, rootLB.Name, c.GetCloudletExternalNetwork(), user)
	if err != nil {
		return nil, err
	}
