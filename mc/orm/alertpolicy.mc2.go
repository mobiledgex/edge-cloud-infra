...
func CreateAlertPolicy(c echo.Context) error {
	...
	resp, err := ctrlapi.CreateAlertPolicyObj(ctx, rc, obj, conn)
	if err != nil {
...
func DeleteAlertPolicy(c echo.Context) error {
	...
	resp, err := ctrlapi.DeleteAlertPolicyObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func UpdateAlertPolicy(c echo.Context) error {
	...
	resp, err := ctrlapi.UpdateAlertPolicyObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func ShowAlertPolicy(c echo.Context) error {
	...
	err = ctrlapi.ShowAlertPolicyStream(ctx, rc, obj, conn, authz.Ok, cb)
	if err != nil {
		return err
	}
	return nil
