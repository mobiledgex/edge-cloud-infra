...
func TestDataConversions(t *testing.T) {
	...
	obj1.Region = "local"
	obj1.Cloudlet.Key.Name = "tmus-cloud-1"
...
func TestMcctlParseErrors(t *testing.T) {
	...
	args = []string{
		...
		"cloudlets:0.key.name=munich",
		"cloudlets:1.key.organization=TDG",
		"cloudlets:1.key.name=dusseldorf",
	}
	out, err = mcctlTest()
	require.NotNil(t, err)
	require.Equal(t, "Error: parsing arg \"cloudlets:.key.organization=TDG\" failed: invalid argument: key \"cloudlets:.key.organization\" not found\n", out)
