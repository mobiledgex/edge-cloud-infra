...
func SetPowerState(c echo.Context) error {
	ctx := GetContext(c)
	rc := &RegionContext{}
	claims, err := getClaims(c)
	if err != nil {
	...
	span := log.SpanFromContext(ctx)
	span.SetTag("org", in.ExecRequest.AppInstKey.AppKey.DeveloperKey.Name)
	resp, err := SetPowerStateObj(ctx, rc, &in.ExecRequest)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func SetPowerStateObj(ctx context.Context, rc *RegionContext, obj *edgeproto.ExecRequest) (*edgeproto.ExecRequest, error) {
	if !rc.skipAuthz && !authorized(ctx, rc.username, obj.AppInstKey.AppKey.DeveloperKey.Name,
		ResourceAppInsts, ActionManage) {
		return nil, echo.ErrForbidden
	}
	...
	api := edgeproto.NewExecApiClient(rc.conn)
	return api.SetPowerState(ctx, obj)
}
