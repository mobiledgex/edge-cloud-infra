...
func CreateResTagTable(c echo.Context) error {
	...
	resp, err := ctrlapi.CreateResTagTableObj(ctx, rc, obj, conn)
	if err != nil {
...
func DeleteResTagTable(c echo.Context) error {
	...
	resp, err := ctrlapi.DeleteResTagTableObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func UpdateResTagTable(c echo.Context) error {
	...
	resp, err := ctrlapi.UpdateResTagTableObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func ShowResTagTable(c echo.Context) error {
	...
	err = ctrlapi.ShowResTagTableStream(ctx, rc, obj, conn, authz.Ok, cb)
	if err != nil {
		return err
	}
	return nil
...
func AddResTag(c echo.Context) error {
	...
	resp, err := ctrlapi.AddResTagObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func RemoveResTag(c echo.Context) error {
	...
	resp, err := ctrlapi.RemoveResTagObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func GetResTagTable(c echo.Context) error {
	...
	resp, err := ctrlapi.GetResTagTableObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
