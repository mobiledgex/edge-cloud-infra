...
func TestServer(t *testing.T) {
	...
	// login as new user1
	tokenMisterX, err := mcClient.DoLogin(uri, user1.Name, user1.Passhash, NoOTP, NoApiKey)
	...
	tokenMisterX, err = mcClient.DoLogin(uri, user1.Name, user1.Passhash, otp, NoApiKey)
	...
	otp, err = totp.GenerateCode(resp.TOTPSharedKey, time.Now())
	require.Nil(t, err, "generate otp")
	tokenMisterY, err := mcClient.DoLogin(uri, user2.Name, user2.Passhash, otp, NoApiKey)
	...
	otp, err = totp.GenerateCode(resp.TOTPSharedKey, time.Now())
	require.Nil(t, err, "generate otp")
	tokenAdmin, err := mcClient.DoLogin(uri, admin.Name, admin.Passhash, otp, NoApiKey)
...
func testLockedUsers(t *testing.T, uri string, mcClient *ormclient.Client) {
	// login as super user
	superTok, err := mcClient.DoLogin(uri, DefaultSuperuser, DefaultSuperpass, NoOTP, NoApiKey)
	require.Nil(t, err, "login as superuser")
	...
	otp, err := totp.GenerateCode(resp.TOTPSharedKey, time.Now())
	require.Nil(t, err, "generate otp")
	_, err = mcClient.DoLogin(uri, user1.Name, user1.Passhash, otp, NoApiKey)
	...
	otp, err = totp.GenerateCode(resp.TOTPSharedKey, time.Now())
	require.Nil(t, err, "generate otp")
	tok1, err := mcClient.DoLogin(uri, user1.Name, user1.Passhash, otp, NoApiKey)
	require.Nil(t, err)
	...
	otp, err = totp.GenerateCode(resp.TOTPSharedKey, time.Now())
	require.Nil(t, err, "generate otp")
	_, err = mcClient.DoLogin(uri, user2.Name, user2.Passhash, otp, NoApiKey)
	require.NotNil(t, err, "login")
	require.Contains(t, err.Error(), "Account is locked")
...
func testPasswordStrength(t *testing.T, ctx context.Context, mcClient *ormclient.Client, uri, token string) {
	...
	require.Nil(t, err, "generate otp")
	_, err = mcClient.DoLogin(uri, adminOld.Name, adminOldPw, otp, NoApiKey)
	require.NotNil(t, err)
	...
	otp, err = totp.GenerateCode(resp.TOTPSharedKey, time.Now())
	require.Nil(t, err, "generate otp")
	_, err = mcClient.DoLogin(uri, user1.Name, user1.Passhash+"1", otp, NoApiKey)
	require.Nil(t, err)
