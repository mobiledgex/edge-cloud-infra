...
func CreateUser(c echo.Context) error {
	...
	if err := ValidateCallbackURL(ctx, createuser.Verify.CallbackURL); err != nil {
		return err
	}
...
func ValidateCallbackURL(ctx context.Context, urlString string) error {
	if urlString == "" {
		return nil
	}
	config, err := getConfig(ctx)
	...
	if !strings.HasSuffix(u.Hostname(), config.ConsoleDomain) {
		return fmt.Errorf("Invalid callback URL domain, must be %s", config.ConsoleDomain)
	}
	return nil
}
...
func UpdateUser(c echo.Context) error {
	...
	if sendVerify {
		if err := ValidateCallbackURL(ctx, cuser.Verify.CallbackURL); err != nil {
			return err
		}
