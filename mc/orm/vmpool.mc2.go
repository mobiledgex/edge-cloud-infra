...
func CreateVMPool(c echo.Context) error {
	...
	resp, err := ctrlapi.CreateVMPoolObj(ctx, rc, obj, conn)
	if err != nil {
...
func DeleteVMPool(c echo.Context) error {
	...
	resp, err := ctrlapi.DeleteVMPoolObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func UpdateVMPool(c echo.Context) error {
	...
	resp, err := ctrlapi.UpdateVMPoolObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func ShowVMPool(c echo.Context) error {
	...
	err = ctrlapi.ShowVMPoolStream(ctx, rc, obj, conn, authz.Ok, cb)
	if err != nil {
		return err
	}
	return nil
...
func AddVMPoolMember(c echo.Context) error {
	...
	resp, err := ctrlapi.AddVMPoolMemberObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
...
func RemoveVMPoolMember(c echo.Context) error {
	...
	resp, err := ctrlapi.RemoveVMPoolMemberObj(ctx, rc, obj, conn)
	if err != nil {
		if st, ok := status.FromError(err); ok {
			err = fmt.Errorf("%s", st.Message())
		}
