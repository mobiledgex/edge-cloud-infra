...
type Api interface {
	...
	ShowAppMetrics(uri, token string) (*ormapi.AllMetrics, int, error)
	ShowClusterMetrics(uri, token string) (*ormapi.AllMetrics, int, error)
	ShowCloudletMetrics(uri, token string) (*ormapi.AllMetrics, int, error)
