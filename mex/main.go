...
func main() {
	...
	if *stack != "" {
		...
		mfVals := &mf.Values
		if err := mexos.InternVaultEnv(mf); err != nil {
			log.FatalLog("cannot intern env", "error", err, "mfVals", mfVals)
		}
		if err := mexos.CheckPlatformEnv(mfVals.OperatorKind); err != nil {
...
func getNewRootLB(mf *mexos.Manifest) error {
	_, err := mexos.NewRootLBManifest(mf)
	if err != nil {
		return err
	}
...
func callOps(kind, op string, mf *mexos.Manifest) error {
...
func tmplUnmarshal(tn, tf string, mf *mexos.Manifest, mfv *mexos.ValueDetail) error {
