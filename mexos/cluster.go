...
func CreateCluster(rootLBName string, clusterInst *edgeproto.ClusterInst) (reterr error) {
	...
	if err != nil {
		log.DebugLog(log.DebugLevelMexos, "invalid cluster flavor", "flavor", flavorName)
		return err
	}

	err = HeatCreateClusterKubernetes(clusterInst, cf)
	if err != nil {
		return err
	}
	...
	ready := false
	for i := 0; i < 10; i++ {
		ready, err = IsClusterReady(clusterInst, cf, rootLBName)
		if err != nil {
			return err
		}
		if ready {
