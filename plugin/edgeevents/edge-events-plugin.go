...
var AppInstsStruct AppInsts

// Add Client connected to specified AppInst to Map
func AddClientKey(ctx context.Context, appInstKey edgeproto.AppInstKey, addr net.Addr, mgr *dmecommon.EdgeEventPersistentMgr) {
	if AppInstsStruct.AppInstsMap == nil {
		log.SpanLog(ctx, log.DebugLevelInfra, "initializing app insts map")
		AppInstsStruct.AppInstsMap = make(map[edgeproto.AppInstKey]Clients)
	}

	clients, ok := AppInstsStruct.AppInstsMap[appInstKey]
	if !ok {
	...
	AppInstsStruct.AppInstsMap[appInstKey] = clients
}

// Remove Client connected to specified AppInst from Map
func RemoveClientKey(ctx context.Context, appInstKey edgeproto.AppInstKey, addr net.Addr) {
	clients, ok := AppInstsStruct.AppInstsMap[appInstKey]
	if !ok {
...
func RemoveAppInstKey(ctx context.Context, appInstKey edgeproto.AppInstKey) {
	_, ok := AppInstsStruct.AppInstsMap[appInstKey]
	if !ok {
		log.SpanLog(ctx, log.DebugLevelInfra, "unable to find appinst to remove", "appInstKey", appInstKey)
		return
	}

	delete(AppInstsStruct.AppInstsMap, appInstKey)
}
...
func SendLatencyRequestEdgeEvent(ctx context.Context, appInst *dmecommon.DmeAppInst, appInstKey edgeproto.AppInstKey) {
	clients, ok := AppInstsStruct.AppInstsMap[appInstKey]
	if !ok {
	...
	for addr, mgr := range clients.ClientsMap {
		go func(addr net.Addr, mgr *dmecommon.EdgeEventPersistentMgr) {
			...
			SendEdgeEventToClient(ctx, latencyRequestEdgeEvent, mgr)
		}(addr, mgr)
	}
}
...
func ProcessLatencySamples(ctx context.Context, appInstKey edgeproto.AppInstKey, addr net.Addr, samples []float64) (*dme.Latency, bool) {
	clients, ok := AppInstsStruct.AppInstsMap[appInstKey]
	if !ok {
		log.SpanLog(ctx, log.DebugLevelInfra, "cannot find appinst, no clients connected to appinst have edge events connection", "appInstKey", appInstKey)
		return nil, false
	...
	SendEdgeEventToClient(ctx, latencyEdgeEvent, mgr)
	return latency, true
}
...
func SendAppInstStateEvent(ctx context.Context, appInst *dmecommon.DmeAppInst, appInstKey edgeproto.AppInstKey, eventType dme.ServerEdgeEvent_ServerEventType) {
	clients, ok := AppInstsStruct.AppInstsMap[appInstKey]
	if !ok {
		log.SpanLog(ctx, log.DebugLevelInfra, "cannot find appinst. no clients connected to appinst have edge events connection.", "appInstKey", appInstKey)
		return
	...
	for client, mgr := range clients.ClientsMap {
		...
		SendEdgeEventToClient(ctx, updateServerEdgeEvent, mgr)
	}
}

// Send ServerEdgeEvent to specified client via persistent grpc stream
func SendEdgeEventToClient(ctx context.Context, serverEdgeEvent *dme.ServerEdgeEvent, mgr *dmecommon.EdgeEventPersistentMgr) {
	mgr.Mux.Lock()
